...
###############
# changes numbers of imputations for MI from 50 to 20 to make the CI generations run in a sensible timeframe
...
handle_missingness_continuous <- function(data, method) {
  ...
  switch(method,
         ...
         "multiple_imputation" = {
           ...
           data_mi <- lapply(seq(1:n_imp), function(i) {
             complete(imputation, i)
           })
           message(paste0("Missing imputation completed - returning ", n_imp, " 50 imputed datasets"))
           list(data = data_mi)
         },
         "none" = {
