##########
# title: figures for the missing-covariate study, binary outcome
##########
# The diagnostic counterpart to results_processing/thesis_figures/miss_bin.R:
# that script picks the two or three panels the chapter prints, this one shows
# every metric bin_miss_metrics.R computes. Figures go to results/all_figures/,
...
# scale: get_binary_oracle_info() wraps its formula in plogis(), which is why
# bin_miss_models.R passes oracle_link = "identity" (the opposite convention to
# binary/).
...
fig_path <- file.path(dirname(path), "results", "all_figures", "missing", "binary")
...
save_fig("bin_miss_bias_all.png", fig_path)

bias_sum_plot <- point_range_plot(metrics_summary, "bias", "Bias")
save_fig("bin_miss_bias_summary.png", fig_path)
...
save_fig("bin_miss_mae_all.png", fig_path)
...
save_fig("bin_miss_corr_all.png", fig_path)

corr_sum_plot <- point_range_plot(corr_summary, "corr", "Pearson correlation",
                                  facet_scales = "free_y")
save_fig("bin_miss_corr_summary.png", fig_path)
...
spearman_plot <- miss_box_plot(corr_metrics, "spearman", "Spearman correlation",
                               hline = NULL, facet_scales = "free_y")
save_fig("bin_miss_spearman_all.png", fig_path)

spearman_sum_plot <- point_range_plot(corr_summary, "spearman",
                                      "Spearman correlation",
                                      facet_scales = "free_y")
save_fig("bin_miss_spearman_summary.png", fig_path)
...
true_cate_tests <- readRDS(file.path(res_path, "bin_miss_true_cate_tests.RDS")) %>%
  apply_labels(MISS_SCENARIO_LABELS) %>%
  mutate(n = factor(n), prop = factor(prop)) %>%
  droplevels()
...
save_fig("bin_miss_blp_summary.png", fig_path)
...
indep_cate_plot <- miss_box_plot(bind_true_cate(metrics, true_cate_raw, "indep_cate"),
                                 ...
                                 facet_scales = "free_x")
save_fig("bin_miss_indep_cate_all.png", fig_path)
...
BLP_power_plot <- point_range_plot(
  ...
  geom_hline(yintercept = 0.05, linetype = "dashed")
save_fig("bin_miss_blp_power.png", fig_path)
...
indep_cate_power_plot <- point_range_plot(
  ...
  geom_hline(yintercept = 0.05, linetype = "dashed")
save_fig("bin_miss_indep_cate_power.png", fig_path)

indep_po_power_plot <- point_range_plot(metrics_summary, "power_indep_po", "Power",
                                        facet_scales = "free_x") +
  geom_hline(yintercept = 0.05, linetype = "dashed")
save_fig("bin_miss_indep_po_power.png", fig_path)
...
write_csv(headline, file.path(res_path, "bin_miss_headline.csv"))
