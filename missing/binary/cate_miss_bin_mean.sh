...
#PBS -J 1-7200
#PBS -N missing_bin_mean
#PBS -o /rds/general/user/evanvogt/projects/nihr_drf_simulations/live/scripts/drf_sims/missing/binary/logs_mean
#PBS -e /rds/general/user/evanvogt/projects/nihr_drf_simulations/live/scripts/drf_sims/missing/binary/logs_mean
...
SAMPLE_IDX=$((IDX / 2400))              # 2400 = 4×3×2×100
REMAINING=$((IDX % 2400))
...
cd "/rds/general/user/evanvogt/projects/nihr_drf_simulations/live/scripts/drf_sims/missing/binary"

# Run the R script with all parameters
Rscript cate_analysis_miss_bin.R "$SCENARIO" "$SAMPLE_SIZE" "$SIM_RUN" "$MISS_TYPE_VAL" "$MISS_PROP_VAL" "mean_imputation"