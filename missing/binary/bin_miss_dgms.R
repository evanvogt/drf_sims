...
generate_binary_scenario_data <- function(scenario, n, mech, return_truth = TRUE) {
  ...
  treatment_effect <- switch(scenario,
                             rep(bW, n),
                             bW + params$b3 * X3 + if(mech == "MNAR-Y") params$bU * U else 0,
                             bW + params$b3 * X3 + params$b4 * X4 + if(mech == "MNAR-Y") params$bU * U else 0,
                             bW + params$b3 * X3 + params$b4 * X4 + params$b45 * X4 * X5 + if(mech == "MNAR-Y") params$bU * U else 0,
  ...
  if(return_truth) {
    ...
    p1 <- p0 + treatment_effect - if(mech == "MNAR-Y") params$bU * U else 0 # marginal is just b_W  + ... + b_U*(E[U]=0) (U generated independently from the Xs)
    tau <- p1 - p0
    
    truth <- data.frame(p0 = p0, p1 = p1, tau = tau)
    if (mech %in% c("MNAR", "MNAR-Y")) truth$U <- U
    result$truth <- truth
  }
  return(result)
}
...
introduce_missingness_binary <- function(data, type, prop, mech, U = NULL) {
  ...
  result <- ampute(data,
                   prop = prop,
                   patterns = indicators,
                   weights = if (mech %in% c("MNAR", "MNAR-Y")) weights else NULL)
...
handle_missingness_binary <- function(data, method) {
  ...
  method <- match.arg(method, c("complete_cases", "mean_imputation", "missforest", "regression", "missing_indicator", "IPW", "multiple_imputation", "none"))
