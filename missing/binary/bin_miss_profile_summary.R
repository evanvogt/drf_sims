##########
# title: turn the profiling runs into PBS directives for bin_miss_1.sh
##########
# Reads the prof_*.RDS files written by bin_miss_profile.R and prints the
...
# binary/bin_profile_summary.R; see that file for the parts that are
# unchanged, and bin_miss_profile.R's header for why this study needs an
# outer(method/scenario/mechanism/run)/inner(workers/grf_threads) split in
# the first place.
#
# Differences from bin_profile_summary.R, both following from that split:
#
...
#    `elapsed` alone - binary/continuous don't need this because their data
#    generation is free.
#
# 3. A new "cost by method" table reports data_gen_elapsed pooled over
#    scenario/mechanism/run - the diagnostic this study specifically needs,
#    since bin_miss_config.R's MISS_METHODS_STUDY spans cheap methods
...
# paths
path <- here()
prof_path <- file.path(dirname(path), "results", "missing", "binary", "profiling")
jobscript <- here("missing", "binary", "jobscripts", "bin_miss_1.sh")
...
# across each outer row's 6 inner cells. Cells where bin_miss_profile.R's
...
# the array runs throttled at %190 (same as binary's bin_1.sh), so it is
...
# since one PBS resource line serves every arm in the array)
...
# SUM (bin_miss_profile.R's header explains why), so they are not comparable
...
# usage tibble came back empty - see bin_miss_profile.R's header on syrup
...
tryCatch({
  ...
  ggsave("bin_miss_profile_timeline.png", plot = timeline_plot, path = prof_path,
         width = 21, height = 15, units = "cm")
  cat(sprintf("\ntimeline plot written to %s\n", file.path(prof_path, "bin_miss_profile_timeline.png")))
}, error = function(e) {
...
# bin_miss_profile.R's header describes - on a real HPC node with a working ps,
...
# printed, or silently write "mem=-Infgb" into the real jobscript.
...
cat(sprintf("Rscript bin_miss_analysis.R \"$PBS_ARRAY_INDEX\" %d %d\n\n",
            best$workers, best$grf_threads))

# ---- write the directives into bin_miss_1.sh --------------------------------
# workers/grf_threads are written as trailing args on the Rscript line, not as
# a manual edit to bin_miss_analysis.R's defaults, so the PBS resource request
# and the R-level parallelism config can never drift apart. Skipped entirely
# if mem_gb couldn't be sized - a walltime-only rewrite would leave a stale
# mem= untouched next to a fresh walltime, silently mismatched.
if (is.na(mem_gb)) {
  ...
  lines <- sub('^Rscript bin_miss_analysis\\.R "\\$PBS_ARRAY_INDEX".*$',
               sprintf('Rscript bin_miss_analysis.R "$PBS_ARRAY_INDEX" %d %d',
                       best$workers, best$grf_threads),
               lines)
  writeLines(lines, jobscript)
  cat(sprintf("directives and workers/grf_threads written into %s\n", jobscript))
} else {
  warning("bin_miss_1.sh not found - directives printed only")
}
