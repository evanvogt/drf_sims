...
# sees all 8 and plan(multisession, workers = 4) doesn't fail; bin_miss_profile.R
...
# by ompthreads.
#
# #PBS -J 1-72, not 1-48 like binary/bin_profile.sh: bin_miss_profile.R uses a
...
# missingness handling pass across them (see bin_miss_profile.R's header).
# Walltime is longer than bin_profile.sh's for the same reason: each task
# does 6 sequential model-fitting cells plus, for missforest/multiple_imputation,
...
Rscript bin_miss_profile.R "$PBS_ARRAY_INDEX"
