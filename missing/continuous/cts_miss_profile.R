##########
# title: resource profiling for the missing-continuous array job
##########
# Runs one complete replicate through the same orchestrator cts_miss_analysis.R
# uses (generate_and_process_continuous_data / generate_continuous_scenario_data ->
...
# timings, memory and CPU so the PBS directives on cts_miss_1.sh/cts_miss_2.sh
# can be measured rather than guessed. Parallel to missing/binary/bin_miss_profile.R
# and continuous/cts_profile.R; see those files' headers for the syrup/multisession
# rationale, not repeated here.
#
#   Rscript missing/continuous/cts_miss_profile.R 1
#
# The specific thing this settles: cts_miss_1.sh/cts_miss_2.sh have always
# requested ompthreads=2 alongside cts_miss_analysis.R's hardcoded workers <- 2,
# but num.threads was never set anywhere in cts_miss_models.R - so grf defaulted
# to all visible cores per multisession worker regardless of what was
# allocated. cate_methods(), cts_miss_models.R and cts_miss_analysis.R now
...
# bin_miss_profile.R answered for missing/binary.
#
# TWO differences from bin_profile.R/cts_profile.R, both driven by this
# study's method axis (cts_miss_config.R's MISS_METHODS_STUDY, 9 values) -
# identical to bin_miss_profile.R's rationale, not repeated in full here:
#
# 1. Data generation is NOT free here. generate_and_process_continuous_data()
#    calls mice()/missForest()/etc *before* plan(multisession) is even set up
#    - it is serial, single-threaded, and independent of (workers,
#    grf_threads). This script uses the same two-level grid as
#    bin_miss_profile.R: an OUTER grid of (method, scenario, mechanism, run) -
#    one PBS array index each - generates/processes data ONCE per outer row
#    (timed with plain system.time()), then an INNER grid of (workers,
#    grf_threads) loops over that same processed data, syrup-wrapping only the
#    parallel model-fitting phase per inner cell.
#
# 2. method == "multiple_imputation" doesn't call run_all_cate_methods() once
#    - cts_miss_analysis.R's own multiple_imputation branch calls future_map()
...
source(here("missing", "continuous", "cts_miss_models.R"))  # run_all_cate_methods -> cate_methods, combine_mi
source(here("missing", "continuous", "cts_miss_config.R"))  # MISS_METHODS_STUDY
...
# scenario 1 in cts_miss_config.R's own grid anyway.
...
# fixed by cts_miss_config.R's grid - this study does not sweep n/type/prop
n <- 500
type <- "both"
prop <- 0.3
n_folds <- 10
...
for (j in seq_len(nrow(inner_params))) {
  ...
  # own libraries have already initialised - matches bin_miss_profile.R
  ...
  cell <- tryCatch({
    ...
    # persist. Mirrors bin_miss_profile.R's single-cell wrap; the branching inside
    # matches cts_miss_analysis.R's three method branches exactly.
    usage <- syrup::syrup({
      total <- system.time({
        if (method == "complete_data") {
...
output_dir <- file.path(dirname(path), "results", "missing", "continuous", "profiling")
