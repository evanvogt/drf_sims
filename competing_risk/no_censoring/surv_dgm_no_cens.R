...
generate_csh_data <- function(scenario, n, return_truth = TRUE, seed = NULL) {
  ...
  if(return_truth) {
    ...
    rmst_sdh_1 <- numeric(n)
    rmst_sdh_0 <- numeric(n)
    ...
    for (i in 1:n) {
      ...
      # subdistribution hazard at time t
      sdh_1 <- function(t) h1_1(t) / (1 + (CIF2_1(t)/S_all_1(t)))
      sdh_0 <- function(t) h1_0(t) / (1 + (CIF2_0(t)/S_all_0(t)))
      
      # subdistribution survival functions
      S_sdh_1 <- Vectorize(function(t) exp(-integrate(sdh_1, 0, t)$value))
      S_sdh_0 <- Vectorize(function(t) exp(-integrate(sdh_0, 0, t)$value))
      
      # RMST integration 
      rmst_csh1_1[i] <- admin - integrate(function(u) CIF1_1(u), 0, admin)$value
      rmst_csh1_0[i] <- admin - integrate(function(u) CIF1_0(u), 0, admin)$value
      rmst_csh2_1[i] <- admin - integrate(function(u) CIF2_1(u), 0, admin)$value
      rmst_csh2_0[i] <- admin - integrate(function(u) CIF2_0(u), 0, admin)$value
      
      rmst_sdh_1[i] <- integrate(S_sdh_1, 0, admin)$value
      rmst_sdh_0[i] <- integrate(S_sdh_0, 0, admin)$value
    }
    
    tau_csh_1 <- rmst_csh1_1 - rmst_csh1_0
    tau_csh_2 <- rmst_csh2_1 - rmst_csh2_0
    
    tau_sdh <- rmst_sdh_1 - rmst_sdh_0
    ...
    truth <- data.frame(
      ...
      rmst_control_sdh = rmst_sdh_0,
      rmst_treat_sdh = rmst_sdh_1,
      tau_sdh = tau_sdh,
