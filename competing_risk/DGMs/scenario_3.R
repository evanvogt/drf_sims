###############
# title: simulating data for scenario 3 - cts HTE
...
generate_dataset <- function(n) {
  ...
  Y <- b0 + b1*X1 + b2*X2 + W*(bW + b4*X4) + err

  p0 <- b0 + b1*X1 + b2*X2
  p1 <- b0 + b1*X1 + b2*X2 + (bW + b4*X4)
  tau <- p1 - p0
...
for (size in sizes) {
  ...
  saveRDS(dataset, file = paste0("live/data/competing_risk//scenario_3_", size, ".RDS"))
}
...
saveRDS(oracle_list, file = paste0("live/data/competing_risk//scenario_3_oracle.RDS"))
...
saveRDS(gates, paste0("live/data/competing_risk//scenario_3_true_GATEs", size, ".RDS"))
