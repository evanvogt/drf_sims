###############
# title: simulating data for scenario 6 - two-way interaction with HTE
...
generate_dataset <- function(n) {
  ...
  lp <- b0 + b1*X1 + b2*X2 + W*(bW + b3*X3 + b4*X4 + b34*X3*X4)
  prob <- plogis(lp)
  Y <- rbinom(n, 1, prob)
  
  p0 <- plogis(b0 + b1*X1 + b2*X2)
  p1 <- plogis(b0 + b1*X1 + b2*X2 + (bW + b3*X3 + b4*X4 + b34*X3*X4))
  tau <- p1 - p0
...
for (size in sizes) {
  ...
  cat(paste0("saving new dataset scenario 6 ", size))
  saveRDS(dataset, file = paste0("live/data/binary/scenario_6_", size, ".RDS"))
  ...
  saveRDS(oracle_list, file = paste0("live/data/binary/scenario_6_", size, "_oracle.RDS"))
}
