...
for (size in sizes) {
  ...
  cat(paste0("saving new dataset scenario 10 ", size))
  saveRDS(dataset, file = paste0("live/data/binary/scenario_10_", size, ".RDS"))
  
  # save the true DGM function for the oracle DR learner
  fmla <- "b0 + b1*X$X1 + b2*X$X2 + W*(bW + b4*exp(X$X4))"
  oracle_list <- list(fmla = fmla, b0 = b0, b1 = b1, b2 = b2, b4 = b4, bW = bW)
  saveRDS(oracle_list, file = paste0("live/data/binary/scenario_10_", size, "_oracle.RDS"))
}
