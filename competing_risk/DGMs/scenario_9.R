###############
# title: simulating data for scenario 9 - wibbly HTE
...
generate_dataset <- function(n) {
  ...
  lp <- b0 + b1*X1 + b2*X2 + W*(bW + b4*cos(X4))
  prob <- plogis(lp)
  Y <- rbinom(n, 1, prob)
  
  p0 <- plogis(b0 + b1*X1 + b2*X2)
  p1 <- plogis(b0 + b1*X1 + b2*X2 + (bW + b4*cos(X4)))
  tau <- p1 - p0
...
for (size in sizes) {
  ...
  cat(paste0("saving new dataset scenario 9 ", size))
  saveRDS(dataset, file = paste0("live/data/binary/scenario_9_", size, ".RDS"))
  
  # save the true DGM function for the oracle DR learner
  fmla <- "b0 + b1*X$X1 + b2*X$X2 + W*(bW + b4*cos(X$X4))"
  oracle_list <- list(fmla = fmla, b0 = b0, b1 = b1, b2 = b2, b4 = b4, bW = bW)
  saveRDS(oracle_list, file = paste0("live/data/binary/scenario_9_", size, "_oracle.RDS"))
}
