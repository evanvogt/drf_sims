...
generate_dataset <- function(n) {
  ...
  Y <- b0 + b1*X1 + b2*X2 + W*(bW + b4*cos(X4)) + err

  p0 <- b0 + b1*X1 + b2*X2
  p1 <- b0 + b1*X1 + b2*X2 + (bW + b4*cos(X4))
  tau <- p1 - p0
...
for (size in sizes) {
  ...
  saveRDS(dataset, file = paste0("live/data/competing_risk//scenario_9_", size, ".RDS"))
}
...
saveRDS(oracle_list, file = paste0("live/data/competing_risk//scenario_9_oracle.RDS"))
