###############
# title: simulating data for scenario 4 - two HTE drivers 1 cts and 1 binary
...
generate_dataset <- function(n) {
  ...
  Y <- b0 + b1*X1 + b2*X2 + W*(bW + b3*X3 + b4*X4) + err
  
  p0 <- b0 + b1*X1 + b2*X2
  p1 <- b0 + b1*X1 + b2*X2 + (bW + b3*X3 + b4*X4)
  tau <- p1 - p0
...
for (size in sizes) {
  ...
  saveRDS(dataset, file = paste0("live/data/competing_risk//scenario_4_", size, ".RDS"))
}


# save the true DGM function for the oracle DR learner
fmla <- "b0 + b1*X$X1 + b2*X$X2 + W*(bW + b3*X$X3 + b4*X$X4)"
oracle_list <- list(fmla = fmla, b0 = b0, b1 = b1, b2 = b2, b3 = b3, b4 = b4, bW = bW)
saveRDS(oracle_list, file = paste0("live/data/competing_risk//scenario_4_oracle.RDS"))
...
# saveRDS(gates, paste0("live/data/competing_risk//scenario_4_true_GATEs", size, ".rds"))
