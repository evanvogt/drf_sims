...
generate_dataset <- function(n) {
  ...
  dataset <- as.data.frame(cbind(Y, W, X1, X2, X3, X4, X5))
...
for (size in sizes) {
  ...
  saveRDS(dataset, file = paste0("live/data/competing_risk//scenario_8_", size, ".RDS"))
}
...
saveRDS(oracle_list, file = paste0("live/data/competing_risk//scenario_8_oracle.RDS"))
