...
pseudo_crossfit <- function(Y, D, horizon, fold_indices, fold_list, pseudo_whole) {
  ...
  pseudos <- future_map(seq_along(fold_list), function(i) {
    ...
    list(ps_RMTL1 = ps_RMTL$pseudo$cause1, ps_RMTL2 = ps_RMTL$pseudo$cause2,
         ps_RMSTc = ps_RMSTc, ps_sh_RMST = ps_sh_RMST,
         in_train = which(in_train), fold = i)
  })
  ...
  list(ps_RMTL1 = ps_RMTL1_mat, ps_RMTL2 = ps_RMTL2_mat,
       ps_RMSTc = ps_RMSTc_mat, ps_sh_RMST = ps_sh_RMST_mat)
}
...
all_cate_surv_models_multiarm <- function(data, Y, D, n_folds = 10, horizon = 30) {
  ...
  prop_per_fold <- future_map(seq_along(fold_list), function(i) {
    fold     <- fold_list[i]
    in_train <- fold_indices != fold
    in_fold  <- !in_train
  ...
  results$ipw <- list()
  results$ipw$RMST1 <- cf_ipw_multiarm(X, Y, D, W, horizon, fold_indices, fold_list, prop_per_fold, arm_levels, all_pairs, event = 1)
  results$ipw$RMST2 <- cf_ipw_multiarm(X, Y, D, W, horizon, fold_indices, fold_list, prop_per_fold, arm_levels, all_pairs, event = 2)
  results$ipw$RMSTc <- cf_ipw_multiarm(X, Y, D, W, horizon, fold_indices, fold_list, prop_per_fold, arm_levels, all_pairs, event = "composite")

  message("Causal Survival Forest pairwise cs approaches (RMST1, RMST2, RMSTc)...")
  results$csf_cs <- list()
  results$csf_cs$RMST1 <- csf_cs_multiarm(X, Y, D, W, horizon, fold_indices, fold_list, prop_per_fold, arm_levels, all_pairs, event = 1)
  results$csf_cs$RMST2 <- csf_cs_multiarm(X, Y, D, W, horizon, fold_indices, fold_list, prop_per_fold, arm_levels, all_pairs, event = 2)
  results$csf_cs$RMSTc <- csf_cs_multiarm(X, Y, D, W, horizon, fold_indices, fold_list, prop_per_fold, arm_levels, all_pairs, event = "composite")

  message("Causal Survival Forest pairwise sh approaches (RMST1, RMST2)...")
  results$csf_sh <- list()
  results$csf_sh$RMST1 <- csf_sh_multiarm(X, Y, D, W, horizon, fold_indices, fold_list, prop_per_fold, arm_levels, all_pairs, event = 1)
  results$csf_sh$RMST2 <- csf_sh_multiarm(X, Y, D, W, horizon, fold_indices, fold_list, prop_per_fold, arm_levels, all_pairs, event = 2)
  ...
  results$pseudo_cf$RMTL1 <- pseudo_cf_multiarm(X, pseudo_cv$ps_RMTL1, W, fold_indices, fold_list, prop_per_fold, arm_levels, all_pairs)
  results$pseudo_cf$RMTL2 <- pseudo_cf_multiarm(X, pseudo_cv$ps_RMTL2, W, fold_indices, fold_list, prop_per_fold, arm_levels, all_pairs)
  results$pseudo_cf$RMSTc  <- pseudo_cf_multiarm(X, pseudo_cv$ps_RMSTc,  W, fold_indices, fold_list, prop_per_fold, arm_levels, all_pairs)
  ...
  nuisance_RMTL1 <- nuisance_pseudo_rf_multiarm(X, pseudo_dr_cv$ps_RMTL1, W, pseudo_whole$ps_RMTL1, fold_indices, fold_pairs, arm_levels, all_pairs)
  nuisance_RMTL2 <- nuisance_pseudo_rf_multiarm(X, pseudo_dr_cv$ps_RMTL2, W, pseudo_whole$ps_RMTL2, fold_indices, fold_pairs, arm_levels, all_pairs)
  nuisance_RMSTc  <- nuisance_pseudo_rf_multiarm(X, pseudo_dr_cv$ps_RMSTc,  W, pseudo_whole$ps_RMSTc,  fold_indices, fold_pairs, arm_levels, all_pairs)

  results$pseudo_dr$RMTL1 <- pseudo_dr_rf_multiarm(X, nuisance_RMTL1, fold_indices, fold_list, all_pairs)
  results$pseudo_dr$RMTL2 <- pseudo_dr_rf_multiarm(X, nuisance_RMTL2, fold_indices, fold_list, all_pairs)
  results$pseudo_dr$RMSTc  <- pseudo_dr_rf_multiarm(X, nuisance_RMSTc,  fold_indices, fold_list, all_pairs)
...
cf_ipw_multiarm <- function(X, Y, D, W, horizon, fold_indices, fold_list, prop_per_fold,
                             arm_levels, all_pairs, event = 1) {
  n_obs      <- nrow(X)
  ...
  fold_results <- future_map(seq_along(fold_list), function(i) {
    ...
    weights_0 <- get_ipw_multiarm(X[in_train, ], Y[in_train], D[in_train], W[in_train], horizon, 0)
    ...
    if (event == "composite") {
      ...
      weights_ce <- get_ipw_multiarm(X[in_train, ], Y[in_train], D[in_train], W[in_train], horizon, competing)
    ...
    forest <- multi_arm_causal_forest(
      X[include, ], pmin(Y[include], horizon), W[include],
      W.hat           = W.hat_include,
  ...
  for (r in fold_results) {
    ...
    for (pn in pair_names) result[[pn]][in_fold] <- pairwise[[pn]]
  }
...
csf_cs_multiarm <- function(X, Y, D, W, horizon, fold_indices, fold_list, prop_per_fold,
                             arm_levels, all_pairs, event = 1) {
  n_obs <- nrow(X)

  D_event <- if (event == "composite") as.numeric(D %in% c(1, 2)) else as.numeric(D == event)
    ...
    lapply(seq_along(all_pairs), function(p_idx) {
      ...
      fold_results <- future_map(seq_along(fold_list), function(i) {
        ...
        forest <- causal_survival_forest(
          X[in_train, ], Y[in_train], W_bin, D_event[in_train],
          W.hat = W.hat_kj, target = "RMST", horizon = horizon
        )
      ...
      tau <- rep(NA_real_, n_obs)
      for (r in fold_results) tau[fold_indices == r$fold] <- r$tau
      tau
    }),
    pair_names
  )
...
csf_sh_multiarm <- function(X, Y, D, W, horizon, fold_indices, fold_list, prop_per_fold,
                             arm_levels, all_pairs, event = 1) {
  n_obs <- nrow(X)
  ...
  setNames(
    lapply(seq_along(all_pairs), function(p_idx) {
      ...
      fold_results <- future_map(seq_along(fold_list), function(i) {
        ...
        if (cens) {
          weights_0      <- get_ipw_multiarm(X[include, ], Y[include], D[include], W[include], horizon, 0)
          total_observed <- weights_0$observed
          sample_weights <- weights_0$ipw[total_observed]
        ...
        forest <- causal_survival_forest(
          X[include, ], Y_sh[include], W_bin, D_sh[include],
          W.hat          = W.hat_kj,
          target         = "RMST", horizon = horizon,
          sample.weights = sample_weights
        )
...
pseudo_cf_multiarm <- function(X, pseudo_mat, W, fold_indices, fold_list, prop_per_fold,
  ...
  fold_results <- future_map(seq_along(fold_list), function(i) {
    ...
    forest <- multi_arm_causal_forest(
      X[in_train, ], pseudo_mat[in_train, fold],
      W[in_train],   W.hat = W.hat_train
    )
  ...
  for (r in fold_results) {
    in_fold <- fold_indices == r$fold
    for (pn in pair_names) result[[pn]][in_fold] <- r$pairwise[[pn]]
  }
  result
}
...
nuisance_pseudo_rf_multiarm <- function(X, pseudo_mat, W, pseudo_whole,
                                         fold_indices, fold_pairs, arm_levels, all_pairs) {
  n_obs      <- nrow(X)
  n_arms     <- length(arm_levels)
  pair_names <- sapply(all_pairs, function(p) pair_name(p[1], p[2]))
  ...
  cross_fits <- future_map(seq_along(fold_pairs), function(i) {
    ...
    mu_model <- regression_forest(cbind(W_dum_train, X_train), pseudo_mat[in_train, i])
  ...
  setNames(
    lapply(pair_names, function(pname) {
      ...
      collate_predictions(fold_list, fold_pairs, fold_indices, cross_fits_pair, "po")
    }),
    pair_names
  )
}
...
pseudo_dr_rf_multiarm <- function(X, po_matrices, fold_indices, fold_list, all_pairs) {
  pair_names <- sapply(all_pairs, function(p) pair_name(p[1], p[2]))
  setNames(
    lapply(pair_names, function(pname) {
