...
once more data has accrued? This study fits CATE estimators on the first
`interim_prop` of a trial and checks whether what they find there — top/bottom
...
```bash
qsub validation/jobscripts/cts_val_1.sh     # 1-300
Rscript validation/cts_val_check.R          # writes failed_ids.txt if any are missing
qsub validation/jobscripts/cts_val_collect.sh
qsub validation/jobscripts/cts_val_metrics.sh
```
