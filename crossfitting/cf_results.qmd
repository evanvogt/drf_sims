...
Everywhere else in `drf_sims`, two-stage estimators fit nuisances over all
`C(V,2)` fold *pairs*: `collate_predictions` (`R/utils.R`) assembles an
...
`V = 10` that is 45 nuisance fits instead of 10 — 4.5x the cost — and it has
never been benchmarked against cheaper alternatives.

**The hypothesis.** Under ordinary (single) crossfitting, a stage-2 training
row `i` carries a pseudo-outcome from a nuisance model trained on every fold
except `i`'s — which *includes* the held-out test fold. Re-randomising the
