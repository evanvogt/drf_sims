...
pseudo-outcome — so an in-sample prediction is pulled towards the unit's own
noise and ends up *further* from the truth, not closer. In this setting
in-sample predictions score **worse** against truth, which is why `naive` is
expected to lose outright rather than look deceptively good. The test score
instead measures how well the fitted surface generalises to fresh covariate
draws — the only like-for-like comparison of the final fitted models across
procedures (see [Generalisation gap](#generalisation-gap)).
...
**DR-learner, SuperLearner (5 arms):** `dcf`, `scf_scf`, `scf_scf_new`,
`scf_full`, `naive` — no OOB analogue exists for SuperLearner, so the OOB
