##########
# title: resource profiling for the binary-outcome array job
##########
# Runs one complete replicate through the same orchestrator bin_analysis.R uses
...
# directives on bin_1.sh can be measured rather than guessed. Parallel to
# continuous/cts_profile.R; see that file's header for the syrup/multisession
# rationale, not repeated here.
#
#   Rscript binary/bin_profile.R 1
#
# The specific thing this settles: bin_1.sh has always requested
# ompthreads=2 alongside bin_analysis.R's hardcoded workers <- 2, but
...
# cate_methods() and bin_analysis.R now accept/forward num.threads; this sweep
# measures whether the 2x2 pairing was actually oversubscribed once that knob
# does something, the same question cf_profile.R and cts_profile.R answered
# for crossfitting and continuous.
#
# Unlike crossfitting (fixed n = 500), binary's single array job spans
# n in {100, 250, 500, 1000} under one #PBS -l line, so n is swept here too -
# sizing must cover the n = 1000 cell, not just a middle value. The binary
# study's actual array grid (bin_config.R) only sweeps scenario in {1, 3, 8, 9},
# not the full 1:10 - the sweep below picks its fewest/most-covariates
# extremes from within that set (scenario 1: no X3-X5; scenario 8: X3+X4+X5).
...
source(here("binary", "bin_models.R"))  # run_all_cate_methods -> cate_methods
...
prof_params <- expand.grid(
  scenario = c(1, 8),      # fewest / most covariates (no X3-X5 vs X3+X4+X5), within {1,3,8,9}
...
# same n-dependent rules as bin_analysis.R, so profiled cost matches production
...
# have already initialised - matches cf_profile.R / cts_profile.R / bin_analysis.R
...
output_dir <- file.path(dirname(path), "results", "binary", "profiling")
