###############
# title: simulating data for scenario 4 - two HTE drivers
...
generate_dataset <- function(n) {
  ...
  lp <- b0 + b1*X1 + b2*X2 + W*(bW + b3*X3 + b4*X4)
  prob <- plogis(lp)
  Y <- rbinom(n, 1, prob)
  
  p0 <- plogis(b0 + b1*X1 + b2*X2)
  p1 <- plogis(b0 + b1*X1 + b2*X2 + (bW + b3*X3 + b4*X4))
  tau <- p1 - p0
...
for (size in sizes) {
  ...
  saveRDS(dataset, file = paste0("live/data/binary/scenario_4_", size, ".RDS"))
}
...
saveRDS(oracle_list, file = paste0("live/data/binary/scenario_4_oracle.RDS"))
...
# saveRDS(gates, paste0("live/data/binary/scenario_4_true_GATEs", size, ".rds"))
