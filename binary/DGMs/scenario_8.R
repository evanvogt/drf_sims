###############
# title: simulating data for scenario 8 - two-way interaction with HTE
...
generate_dataset <- function(n) {
  ...
  lp <- b0 + b1*X1 + b2*X2 + W*(bW + b3*X3 + b4*X4 + b45*X4*X5)
  prob <- plogis(lp)
  Y <- rbinom(n, 1, prob)
  
  p0 <- plogis(b0 + b1*X1 + b2*X2)
  p1 <- plogis(b0 + b1*X1 + b2*X2 + (bW + b3*X3 + b4*X4 + b45*X4*X5))
  tau <- p1 - p0
  ...
  dataset <- as.data.frame(cbind(Y, W, X1, X2, X3, X4, X5, X01, X02, X03, X04, X05))
...
for (size in sizes) {
  ...
  cat(paste0("saving new dataset scenario 8 ", size))
  saveRDS(dataset, file = paste0("live/data/binary/scenario_8_", size, ".RDS"))
  
  # save the true DGM function for the oracle DR learner
  fmla <- "b0 + b1*X$X1 + b2*X$X2 + W*(bW + b3*X$X3 + b4*X$X4 + b45*X$X4*X$X5)"
  oracle_list <- list(fmla = fmla, b0 = b0, b1 = b1, b2 = b2, b3 = b3, b4 = b4, b45 = b45, bW = bW)
  saveRDS(oracle_list, file = paste0("live/data/binary/scenario_8_", size, "_oracle.RDS"))
}
