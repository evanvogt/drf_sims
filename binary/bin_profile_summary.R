##########
# title: turn the profiling runs into PBS directives for bin_1.sh
##########
# Reads the prof_*.RDS files written by bin_profile.R and prints the recommended
...
# scheduler output to parse. Direct port of continuous/cts_profile_summary.R;
# see that file for the parts that are unchanged.
#
# One difference from the crossfitting version: binary's single array job
...
# paths
path <- here()
prof_path <- file.path(dirname(path), "results", "binary", "profiling")
jobscript <- here("binary", "jobscripts", "bin_1.sh")
...
# ---- cost by n ---------------------------------------------------------------
# binary-specific: how much n alone drives cost, pooling over the other axes
...
ggsave("bin_profile_timeline.png", plot = timeline_plot, path = prof_path,
       width = 21, height = 15, units = "cm")
cat(sprintf("\ntimeline plot written to %s\n", file.path(prof_path, "bin_profile_timeline.png")))
...
cat(sprintf("Rscript bin_analysis.R \"$PBS_ARRAY_INDEX\" %d %d\n\n",
            best$workers, best$grf_threads))

# ---- write the directives into bin_1.sh -------------------------------------
# workers/grf_threads are written as trailing args on the Rscript line, not as a
# manual edit to bin_analysis.R's defaults, so the PBS resource request and the
...
if (file.exists(jobscript)) {
  ...
  lines <- sub('^Rscript bin_analysis\\.R "\\$PBS_ARRAY_INDEX".*$',
               sprintf('Rscript bin_analysis.R "$PBS_ARRAY_INDEX" %d %d',
                       best$workers, best$grf_threads),
               lines)
  writeLines(lines, jobscript)
  cat(sprintf("directives and workers/grf_threads written into %s\n", jobscript))
} else {
  warning("bin_1.sh not found - directives printed only")
}
