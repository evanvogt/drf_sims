...
run_dr_superlearner <- function(X, Y, W, fold_indices, fold_list, fold_pairs, workers, sl_lib) {
  ...
  cross_fits <- future_map(seq_along(fold_pairs), function(i) {
    ...
    Y_lib <- pretest_superlearner(Y[in_train], X_W_train, sl_lib, family = binomial())
    Y.hat.model <- SuperLearner(Y[in_train], X_W_train, family = binomial(), SL.library = Y_lib, method = "method.CC_LS")
