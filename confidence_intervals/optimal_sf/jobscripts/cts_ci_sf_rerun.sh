#!/bin/bash
# Reruns only the array indices listed in failed_cts_ids.txt by cts_ci_sf_check.R.
# -J and the resource request are rewritten by check_failed(); the values here
# are what it computes from cts_ci_sf_1.sh, so the first check leaves them alone.
#
...
#
# Needs logs_cts_rerun/ to exist on the cluster - PBS aborts a job whose output
...
Rscript cts_ci_sf_analysis.R "$jobid"
