...
# plan(multisession, workers = 4) doesn't fail; cts_profile.R still sets
# OMP_NUM_THREADS from the sweep grid before spawning its multisession workers,
# so per-worker thread counts are controlled there, not by ompthreads. Walltime
# and mem are generous because the sweep includes n = 1000 with the full
# SuperLearner library (SL.ranger included), the heaviest cell in the study.
#
...
Rscript cts_profile.R "$PBS_ARRAY_INDEX"
