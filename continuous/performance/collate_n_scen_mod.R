...
# path to results
res_dir <- paste0("live/results/continuous/", scenario, "/", n, "/", model)
...
if (length(all_files) == 1000) {
  ...
  scen_num <- gsub("scenario_", "", scenario) %>% as.numeric()
  array_nums <- 1000*(scen_num - 1) + failed_sims %>% sort()
  
  writeLines(as.character(array_nums), paste0("live/scripts/continuous/", model, "/jobscripts/failed_", n, "_", scenario, ".txt"))
