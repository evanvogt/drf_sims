...
generate_dataset <- function(n) {
  ...
  Y <- b0 + b1*X1 + b2*X2 + W*(bW + b3*X3 + b4*X4 + b45*X4*X5) + err
  
  p0 <- b0 + b1*X1 + b2*X2
  p1 <- b0 + b1*X1 + b2*X2 + (bW + b3*X3 + b4*X4 + b45*X4*X5)
  tau <- p1 - p0
...
for (size in sizes) {
  ...
  saveRDS(dataset, file = paste0("live/data/continuous/scenario_8_", size, ".RDS"))
  
  # save the true DGM function for the oracle DR learner
  fmla <- "b0 + b1*X1 + b2*X2 + W*(bW + b3*X3 + b4*X4 + b45*X4*X5)"
  fmla <- gsub("\\b(X\\d+)\\b", "X$\\1", fmla)
  oracle_list <- list(fmla = fmla, b0 = b0, b1 = b1, b2 = b2, b3 = b3, b4 = b4, b45 = b45, bW = bW)
  saveRDS(oracle_list, file = paste0("live/data/continuous/scenario_8_", size, "_oracle.RDS"))
}
